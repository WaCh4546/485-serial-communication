模型计算机，通信使用USART1， PA9接RX, PA10接TX。
功能：将上位机发来的数据输入模型进行计算，模拟实际系统中的被控对象，并对输出进行回传。
接发报文格式：
	接收：轮询指令 C a 0x0d 0x0a
	          数据重发指令 C f 0x0d 0x0a           
	          数据接收 C d 符号位 数据 0x0d 0x0a    例 Cd1255006 crc 0x0d0x0a  收到数据25.5006
	           	     数据采用实际数据*10000后的整型传输，符号位为1数据为正，符号位为2数据为负
	          时间同步 D b 0x0d 0x0a
	          停机指令 D e 0x0d 0x0a  停机，停止接收数据，并清空数据缓存。
	发送：状态应答 0C 03 0x0d 0x0a
	          CRC校验失败 0C 04 0x0d 0x0a
	          回传数据 0C 00 符号位 数据 0x0d 0x0a
...
模拟的被控对象为一阶惯性系统: OUTPUT=INPUT/(1+ST), 其差分方程如下
...
modle.InertiaTime ：惯性时间常数 默认设置为3
modle.SampleTime：采样时间  默认设置为0.4
modle.PidOutput: PID控制量								2021.2.18